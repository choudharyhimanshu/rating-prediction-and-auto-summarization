...
FILE_OUT = open(LOCAL_DIR+'out_pol.txt','a')
...
def getDataset(nrows=100, random=0):
    ...
    if nrows == -1:
        return read_csv(dataset_file,header=0,index_col=0,usecols=['Id','Score','Text','Summary'],encoding='utf-8')
    if random:
        data = read_csv(dataset_file,header=0,index_col=0,usecols=['Id','Score','Text','Summary'],nrows=50*nrows,encoding='utf-8')
        rand = np.random.random_integers(0,49*nrows)
        data = data[rand:rand+nrows]
        return data
    else:
        return read_csv(dataset_file,header=0,index_col=0,usecols=['Id','Score','Text','Summary'],nrows=nrows,encoding='utf-8')
